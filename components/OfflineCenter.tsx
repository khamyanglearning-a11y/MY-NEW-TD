...
const OfflineCenter: React.FC<OfflineCenterProps> = ({ words, packs, onTogglePack, isOnline }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-20">
      <div className="bg-ink rounded-[3.5rem] p-12 text-center relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/60 max-w-lg mx-auto font-medium leading-relaxed">
          ...
          <div className="mt-12 flex justify-center gap-8">
             <div className="text-center">
               ...
               <div className="text-[10px] font-black text-white/40 uppercase tracking-widest mt-1">Saved Words</div>
             </div>
             ...
             <div className="text-center">
               ...
               <div className="text-[10px] font-black text-white/40 uppercase tracking-widest mt-1">Coverage</div>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-6">
          <h3 className="text-xs font-black text-heritage/40 uppercase tracking-[0.3em] pl-4">Download by Category</h3>
          <div className="space-y-3">
            {categories.map((cat: string) => {
              ...
              return (
                <div key={cat} className={`bg-white p-6 rounded-[2rem] border transition-all flex items-center justify-between group ${isFullyDownloaded ? 'border-emerald-100 shadow-sm' : 'border-heritage/10 hover:shadow-xl'}`}>
                  <div className="flex items-center gap-5">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${isFullyDownloaded ? 'bg-emerald-50 text-emerald-600' : 'bg-white border border-heritage/10 text-heritage/40 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                      {cat.charAt(0)}
                    </div>
                    <div>
                      <h4 className="font-black text-ink leading-none mb-1">{cat}</h4>
                      <p className="text-[10px] font-bold text-heritage/40 uppercase tracking-tighter">
                  ...
                  <button 
                    ...
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isFullyDownloaded ? 'bg-emerald-600 text-white' : 'bg-white border border-heritage/10 text-heritage/20 hover:bg-blue-600 hover:text-white disabled:opacity-50'}`}
                  >
                    {isFullyDownloaded ? (
        ...
        <div className="space-y-6">
          <h3 className="text-xs font-black text-heritage/40 uppercase tracking-[0.3em] pl-4">Full Language Packs</h3>
          <div className="space-y-4">
             {[
               ...
               <div key={pack.id} className="bg-white p-8 rounded-[2.5rem] border border-heritage/10 relative overflow-hidden group hover:shadow-2xl transition-all">
                 ...
                 <div className="relative z-10 flex flex-col h-full">
                   <h4 className="text-xl font-black text-ink mb-2">{pack.name}</h4>
                   <p className="text-sm font-medium text-heritage/40 mb-6 leading-relaxed">{pack.desc}</p>
                   ...
                   <div className="mt-auto flex items-center justify-between">
                     ...
                     <button 
                       ...
                       className="px-6 py-3 bg-ink text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-600 transition-all active:scale-95"
                     >
                       Download Pack
                     </button>
                   </div>
