...
const MessageCenter: React.FC<MessageCenterProps> = ({ currentUser, users, messages, dictionary, onSendMessage, onMarkAsRead, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white rounded-none md:rounded-[2.5rem] w-full max-w-6xl h-full md:h-[85vh] shadow-2xl flex overflow-hidden border border-heritage/10 animate-in zoom-in duration-300">
        
        {/* User Sidebar */}
        <div className={`w-full md:w-80 border-r border-heritage/10 flex flex-col bg-white ${selectedUserId ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-6 border-b border-heritage/10 bg-white">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black text-ink tracking-tighter">Community</h2>
              <button onClick={onClose} className="p-2 hover:bg-white border border-heritage/10 rounded-full transition-colors">
                <svg className="w-6 h-6 text-heritage/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            
            <div className="relative">
              <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-heritage/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              <input 
                type="text"
                placeholder="Find a member..."
                value={userSearchQuery}
                onChange={(e) => setUserSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-3 bg-white border border-heritage/10 focus:bg-white focus:border-blue-500 rounded-2xl outline-none text-sm font-bold transition-all shadow-inner"
              />
            </div>
          </div>
              ...
              filteredUsers.map(user => (
                ...
                >
                  ...
                  <div className="text-left min-w-0 flex-1">
                    ...
                    <div className={`text-[10px] truncate uppercase tracking-widest font-bold ${selectedUserId === user.id ? 'text-blue-100' : 'text-heritage/40'}`}>
                      {user.address}
                    </div>
                  </div>
        ...
        <div className={`flex-1 flex-col bg-white ${selectedUserId ? 'flex' : 'hidden md:flex'}`}>
          {selectedUser ? (
            <>
              ...
              {/* Messages Display */}
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 bg-white custom-scrollbar relative">
                ...
                {showMiniDict && (
                  <div className="absolute top-4 right-4 z-20 w-72 bg-white rounded-3xl shadow-2xl border border-blue-100 p-4 animate-in slide-in-from-top-2 duration-200">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button onClick={() => setShowMiniDict(false)}><svg className="w-4 h-4 text-heritage/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg></button>
                    </div>
                    <input 
                      type="text"
                      autoFocus
                      ...
                      className="w-full px-4 py-2 bg-white border border-heritage/10 rounded-xl text-xs font-bold outline-none border border-transparent focus:border-blue-200"
                    />
                    <div className="mt-4 space-y-2">
                      {filteredDict.map(w => (
                        ...
                        >
                          ...
                          <span className="text-[9px] font-bold text-heritage/40">{w.english}</span>
                        </button>
                      ))}
                      {dictSearchQuery && filteredDict.length === 0 && (
                        <p className="text-[9px] text-heritage/40 text-center italic">Word not found in dictionary</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {activeChatMessages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center opacity-30">
                    <div className="w-20 h-20 bg-white border border-heritage/10 rounded-full flex items-center justify-center mb-4">
                      <svg className="w-10 h-10 text-heritage/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                    </div>
                    <p className="text-sm font-black uppercase tracking-widest text-heritage/40">Say hello in Tai Khamyang!</p>
                  </div>
                ) : (
                    ...
                    <div key={msg.id} className={`flex ${msg.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] md:max-w-[70%] p-4 rounded-[1.5rem] shadow-sm text-sm font-medium relative ${
                        msg.senderId === currentUser.id 
                          ...
                          : 'bg-white text-ink rounded-bl-none border border-heritage/10'
                      }`}>
                        {msg.text}
              ...
              {/* Input Area */}
              <div className="p-4 md:p-6 border-t border-heritage/10 bg-white">
                {/* Quick Greetings */}
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-3 md:gap-4">
                  <input 
                    ...
                    className="flex-1 px-5 md:px-6 py-4 bg-white border border-heritage/10 focus:bg-white focus:border-blue-500 rounded-[1.5rem] outline-none text-sm font-bold shadow-inner transition-all"
                  />
                  <button 
                    type="submit"
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-white">
              ...
              <h2 className="text-3xl font-black text-ink tracking-tighter mb-4">Member Connect</h2>
              <p className="text-heritage/40 max-w-sm font-medium leading-relaxed">Choose a community member to start a private conversation or practice your Tai Khamyang skills.</p>
            </div>
          )}
        </div>
      </div>
